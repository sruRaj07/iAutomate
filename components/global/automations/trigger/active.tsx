...
const ActiveTrigger = ({ keywords, type }: Props) => {
  return (
    <div className="bg-background-80 rounded-xl w-full">
        ...
        <div className="flex gap-2 mt-5 flex-wrap">
            {keywords.map((word) => (
                <div 
                key={word.id}
                className="bg-gradient-to-br from-[#3352CC] to-[#1C2D70] flex items-center gap-x-2 capitalize text-white 
                font-light py-1 px-4 rounded-full"
                >
                    <p>{word.word}</p>
                </div>
            ))}
